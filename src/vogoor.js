class Vogoor {
    constructor() { 
        ...
        this.cssFileContent = "";
        this.newCssSelectors = "";
        this.shortClassNames = this.getDefaultShortClassNames();
    }

    getDefaultShortClassNames() {
        return [
            {name: "color", short:"c"},
    ...
    translateClass (str) {
        var prp = str.split(this.propertySeparator);
        var className = this.translateClassName(str);
        ...
        return {name: className, propertyName: prpName, propertyValue: prpVal};
    }

    validCSS(property, value) {
        if (!property) {
            return false; 
        }
    ...
    translateClassName(str) {
        let specChars = ["~", "!", "@", "$", "%", "^", "&",
         "*", "(", ")", "+", "=", ".", "/", "'", ";", ":", 
         '"', "?", ">", "<", "[", "]", "{", "}", "|", "#"];

        for (var i = 0; i < specChars.length; ++i) {
            str = this.replaceAll(str, specChars[i], "\\"+specChars[i]);
        }
    ...
    replaceAll(str, match, replacement){
        return str.replace(new RegExp(this.escapeRegex(match), 'g'), ()=>replacement);
    }
