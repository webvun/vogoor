class UnderscoreCSS {
    constructor(prefix, propertySeparator) {
        ...
        this.shortClassNames = [{ name: "color", short: "c" }, { name: "padding-top", short: "pt" }, { name: "p", short: "padding" }];
        this.init();
    }

    init() {
        document.addEventListener("DOMContentLoaded", event => {
            var els = this.prefix ? document.querySelectorAll("[class*=" + this.prefix + "]") : document.querySelectorAll("[class*=\\:]");
    ...
    findClass(classes) {
        ...
        for (var i = 0; i < classes.length; ++i) {
            if (this.prefix && classes[i].startsWith(this.prefix) && classes[i].includes(this.propertySeparator)) {
                res.push(classes[i]);
            }
